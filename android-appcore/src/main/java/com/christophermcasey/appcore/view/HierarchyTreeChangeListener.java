...
public final class HierarchyTreeChangeListener implements ViewGroup.OnHierarchyChangeListener {
   ...
   */
  public static HierarchyTreeChangeListener wrap(
      @NonNull ViewGroup.OnHierarchyChangeListener delegate) {
    return new HierarchyTreeChangeListener(delegate);
  }

  public static void assignRecursively(@NonNull ViewGroup root,
      @NonNull HierarchyTreeChangeListener hierarchyTreeChangeListener) {
    root.setOnHierarchyChangeListener(hierarchyTreeChangeListener);
    for (int i = 0; i < root.getChildCount(); i++) {
      View child = root.getChildAt(i);
      if (child instanceof ViewGroup) {
        assignRecursively((ViewGroup) child, hierarchyTreeChangeListener);
      }
    }
  }
